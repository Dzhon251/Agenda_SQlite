...
public class DeveloperBD    extends SQLiteOpenHelper {
    ...
    public List<CursosModelos> mostrarCursos(){
        SQLiteDatabase bd=getReadableDatabase();
        Cursor cursor=bd.rawQuery("SELECT * FROM CONTACTOS",null);
        List<CursosModelos> cursos=new ArrayList<>();

        if(cursor.moveToFirst()){
            do{
                cursos.add(new CursosModelos(cursor.getString(0),cursor.getString(1),cursor.getString(2)));
    ...
    public void buscarCursos(CursosModelos cursos,String nombre) {
        SQLiteDatabase bd=getReadableDatabase();
